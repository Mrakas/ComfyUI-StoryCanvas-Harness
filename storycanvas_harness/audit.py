    ...
    for shot in plan.shots:
        ...
        shot_cards.append(
            ...
            <article>
              ...
              <section><h3>Canvas image prompt</h3><pre>{html.escape(shot.image_prompt)}</pre></section>
              <section><h3>Ordered references</h3><ol>{reference_rows or "<li>Text-only keyframe</li>"}</ol></section>
              <section><h3>Final Canvas</h3><p>{html.escape(keyframe.path if keyframe else "pending")}</p></section>
              <section><h3>MiniMax-H3 prompt</h3><pre>{html.escape(shot.h3_prompt)}</pre></section>
              <section><h3>Video</h3><p>{html.escape(video.path if video else "locked or pending")}</p></section>
            </article>
            """.strip()
        )
...
pre{{white-space:pre-wrap;font:13px/1.65 ui-monospace,monospace;background:#f6f6f3;padding:14px}}code{{font-family:ui-monospace,monospace}}ol{{padding-left:24px}}
...
{"".join(shot_cards)}</main></body></html>"""
